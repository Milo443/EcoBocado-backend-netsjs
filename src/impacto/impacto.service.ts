...
export class ImpactoService {
  ...
  async getGlobalImpact(): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException('Error al calcular impacto global');
    }
  }
  ...
  async getDonorDashboard(donorId: string): Promise<any> {
    ...
    try {
      ...
      console.log('[ImpactoService] Peso rescatado:', peso_donador);

      // 2. Lotes actualmente activos - Desde MongoDB
      const lotes_activos = await this.loteModel.countDocuments({
        donante_id: donorId,
        estado: EstadoLote.ACTIVO,
        esta_borrado: { $ne: true },
      });
      console.log('[ImpactoService] Lotes activos:', lotes_activos);

      // 3. Entregas hoy (completadas en las últimas 24h) - Desde PostgreSQL
      const hoy = new Date();
      ...
      console.log('[ImpactoService] Lotes del donador encontrados:', loteIds.length);
      ...
      console.error('[ImpactoService] ERROR en getDonorDashboard:', error);
      throw new InternalServerErrorException(`Error en dashboard: ${error.message}`);
    }
  }
}
